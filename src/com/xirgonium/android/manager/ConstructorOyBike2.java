...
public abstract class ConstructorOyBike2 extends CommonStationManager {

	protected long							lastUpdateTimeStamp		= 0;
	protected Hashtable<String, Station>	lastUpdatedFavorites	= new Hashtable<String, Station>();

	public ConstructorOyBike2() {
	}

	public ConstructorOyBike2(Context launched) {
		super(launched);
	}
	...
	protected void getStationInfoFromPage() throws NoInternetConnection {
		...
		try {
			...
			try {
				...
				NodeList stations = doc.getElementsByTagName("stand");
				...
				for (int i = 0; i < stations.getLength(); i++) {
					...
					//name
					String name = viewEntry.getAttributes().getNamedItem("name").getNodeValue();
					aStation.setName(name);

					//id
					String id = viewEntry.getAttributes().getNamedItem("id").getNodeValue();
					aStation.setId(id);

					//lat
					String lat = getTextNodeValue(viewEntry, "lat");
					aStation.setLatitude(Double.parseDouble(lat));

					//id
					String lng = getTextNodeValue(viewEntry, "lng");
					...
					// slots
					String slots = getTextNodeValue(viewEntry, "ap");
					aStation.setFreeSlot(Integer.parseInt(slots));

					// bikes
					String bikes = getTextNodeValue(viewEntry, "ab");
